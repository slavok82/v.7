...
const averageGrade = (student) => {
    return {
        ...
        marks: student.marks,
        average: (student.marks.reduce((acc, curr) => acc + curr) / student.marks.length)
    };
};
const result = listStudents.map(averageGrade);
console.log(result);  
//--
const dropOutList = (student) => {    
    if (student.average < 60) {
        return student.name;
    };
};
const studentsForExpulsion = result.filter(dropOutList);
console.log(studentsForExpulsion);
//--
const studentsMedian = (student) => {      
    return {
        name: student.name,
        average: Math.round(student.marks.reduce((acc, curr) => acc + curr) / student.marks.length)
    }        
}
const median = listStudents.map(studentsMedian);
console.log(median);  
//--
const addStudent = (arr, name, specialty, marks) => {
    const newStudent = {
        name: name,
        specialty: specialty,
        marks: marks  
...
console.log(addStudent(listStudents, 'Bob Marley', 'Musician', 76));
//--
const sortStudent = median.sort((a, b) =>
b.average - a.average);
